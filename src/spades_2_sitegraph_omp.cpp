...
extern std::atomic_int64_t edgecount = 0;
            ...
            // #pragma omp task
            {
                ...
                if (contig == nullptr) {
                    ...
                    if (contig->sites.size() != 0) {
                        ...
                        // SiteComp); Add edges for sites within the same contig
                    ...
                    // csvdump << contig->name << "," << contig->sites.size() <<
                    // std::endl;
                }
            }
        }
                ...
                if (contig->sites.size() != 0) {
                    ...
                    if (contig->next.size() != 0)
                        for (auto next : contig->next) {
                            if (next != nullptr) {
                                auto site_tuples =
                                    next->get_first_site(overlap_length);
                                for (auto site_tuple : site_tuples) {
                                    auto edge = new Edge(
                                        std::get<0>(site_tuple),
