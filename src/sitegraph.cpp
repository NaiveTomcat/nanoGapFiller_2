                       ...
                       int depth, int length)
{
    if (sites.size() != 0) {
        ...
        if (sites.back()->pos >= overlap_length) {
            ...
            if (simplify_range >= 0) {
                // for (auto path : from->sites.back()->edges) {
                //     if ((path->to->id == site->id) &&
                //         (std::abs(path->len - total_length) <=
                //         simplify_range)) return {};
                // }
                auto pr = from->sites.back()->edgelengths.equal_range(site->id);
                while (pr.first != pr.second) {
                    if ((std::abs(pr.first->second - total_length) <=
                         simplify_range))
                        return {};
                    pr.first++;
                }
        ...
        if (simplify_range < 0)
            ...
            std::copy_if(
                ...
                [&simplify_range, &lengths](
                    const std::tuple<Site *, int64_t, std::vector<std::string>>
                        &v) {
                    // for (auto l : lengths)
                    ...
                    //          simplify_range))
                    //         return false;
                    // lengths.push_back({std::get<0>(v)->id, std::get<1>(v)});
                    auto pr = lengths.equal_range(std::get<0>(v));
                    while (pr.first != pr.second) {
